...
──────────────────────
Generate all figures needed for the report Insights section.

Figures produced (saved to config.PLOT_DIR):
  1. metrics_bar.png        – MAE / RMSE / MAPE bar chart
  2. loss_curves.png        – training & validation loss curves
  3. prediction_overlay.png – actual vs predicted (first 200 test pts)
  4. scatter.png            – predicted vs actual scatter
  5. error_dist.png         – error distribution histogram
"""
...
def plot_metrics_bar(results: list, save: bool = True):
    ...
    ax.set_ylabel("Error")
    ax.set_title("Model evaluation — test-set error metrics")
